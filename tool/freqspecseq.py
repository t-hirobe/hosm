...
parser.add_argument('-t', '--target', metavar='target', type=int,
...
time = 0
dw = 1/20.0
target_w = target*dw + dw*0.5
print "# w = {0} : {1} - {2}".format(target_w, target_w+dw*0.5, target_w + dw*1.5)
while time <= endTime:
    try:
#file.seek(one_time_data_len * seek_num, 1)
        ...
        print time, np.sum(np.array(output[target])), len(output[target])
        sys.stdout.flush()
